...
class Command(BaseCommand):
    ...
    def create_organization(self, count: int) -> None:
        fake = Faker()
    ...
    def create_students(self, count: int) -> None:
        fake = Faker('en-PH')

        for _ in range(count):
            Student.objects.create(
                student_id = f"{fake.random_int(2020,2024)}-{fake.random_int(1, 9)}-{fake.random_int(1, 9)}-{fake.random_int(1, 8)}-{fake.random_number(digits=4)}",
    ...
    def create_membership(self, count: int) -> None:
        fake = Faker()
